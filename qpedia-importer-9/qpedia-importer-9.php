 ...
 * Version:     9.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-9
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI9_CPT', 'quantum_article' );
define( 'QPI9_TAX', 'quantum_category' );
define( 'QPI9_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi9_load_data() {
	$file = QPI9_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi9_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi9_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi9_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI9_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI9_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi9_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI9_TAX );
 ...
 */
function qpi9_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi9_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi9_authors', $cache, 'qpedia', 300 );
	}
...
function qpi9_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi9_resolve_author( $a['meta']['author'] );
	 ...
	 */
	if ( ! empty( $a['category'] ) ) {
		$current = wp_get_object_terms( $id, QPI9_TAX, array( 'fields' => 'ids' ) );
		if ( is_wp_error( $current ) || empty( $current ) ) {
			$ids = qpi9_term_chain( $a['category'], $map );
			if ( $ids ) { wp_set_object_terms( $id, $ids, QPI9_TAX, false ); }
		}
	}
...
function qpi9_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi9_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 9</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi9_run'] ) && check_admin_referer( 'qpi9_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi9_update'] ),
			'draft'  => ! empty( $_POST['qpi9_draft'] ),
		);
		$map = qpi9_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi9_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi9_import' );
	echo '<p><label><input type="checkbox" name="qpi9_update" value="1" checked> به‌روزرسانی مقالات موجود (این بسته فقط همین کار را می‌کند)</label></p>';
	echo '<p><label><input type="checkbox" name="qpi9_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای بازبینی', 'primary', 'qpi9_run' );
	echo '</form>';
