<?php
/**
 * Plugin Name: QPedia Importer 19
 ...
 * Version:     19.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-19
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI19_CPT', 'quantum_article' );
define( 'QPI19_TAX', 'quantum_category' );
define( 'QPI19_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi19_load_data() {
	$file = QPI19_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi19_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi19_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi19_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI19_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI19_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi19_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI19_TAX );
 ...
 */
function qpi19_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi19_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi19_authors', $cache, 'qpedia', 300 );
	}
...
function qpi19_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi19_resolve_author( $a['meta']['author'] );
	 ...
	 */
	if ( ! empty( $a['category'] ) ) {
		$current = wp_get_object_terms( $id, QPI19_TAX, array( 'fields' => 'ids' ) );
		if ( is_wp_error( $current ) || empty( $current ) ) {
			$ids = qpi19_term_chain( $a['category'], $map );
			if ( $ids ) { wp_set_object_terms( $id, $ids, QPI19_TAX, false ); }
		}
	}
...
function qpi19_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi19_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 19</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi19_run'] ) && check_admin_referer( 'qpi19_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi19_update'] ),
			'draft'  => ! empty( $_POST['qpi19_draft'] ),
		);
		$map = qpi19_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi19_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi19_import' );
	echo '<p><label><input type="checkbox" name="qpi19_update" value="1" checked> به‌روزرسانی مقالات موجود (این بسته فقط همین کار را می‌کند)</label></p>';
	echo '<p><label><input type="checkbox" name="qpi19_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای بازبینی', 'primary', 'qpi19_run' );
	echo '</form>';
