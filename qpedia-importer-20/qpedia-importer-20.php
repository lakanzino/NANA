<?php
/**
 * Plugin Name: QPedia Importer 20
 ...
 * Version:     20.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-20
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI20_CPT', 'quantum_article' );
define( 'QPI20_TAX', 'quantum_category' );
define( 'QPI20_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi20_load_data() {
	$file = QPI20_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi20_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi20_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi20_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI20_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI20_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi20_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI20_TAX );
 ...
 */
function qpi20_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi20_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi20_authors', $cache, 'qpedia', 300 );
	}
...
function qpi20_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi20_resolve_author( $a['meta']['author'] );
	 ...
	 */
	if ( ! empty( $a['category'] ) ) {
		$current = wp_get_object_terms( $id, QPI20_TAX, array( 'fields' => 'ids' ) );
		if ( is_wp_error( $current ) || empty( $current ) ) {
			$ids = qpi20_term_chain( $a['category'], $map );
			if ( $ids ) { wp_set_object_terms( $id, $ids, QPI20_TAX, false ); }
		}
	}
...
function qpi20_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi20_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 20</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi20_run'] ) && check_admin_referer( 'qpi20_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi20_update'] ),
			'draft'  => ! empty( $_POST['qpi20_draft'] ),
		);
		$map = qpi20_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi20_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi20_import' );
	echo '<p><label><input type="checkbox" name="qpi20_update" value="1" checked> به‌روزرسانی مقالات موجود (این بسته فقط همین کار را می‌کند)</label></p>';
	echo '<p><label><input type="checkbox" name="qpi20_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای بازبینی', 'primary', 'qpi20_run' );
	echo '</form>';
