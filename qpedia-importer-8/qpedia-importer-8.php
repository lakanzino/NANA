<?php
/**
 * Plugin Name: QPedia Importer 8
 * Description: ایمپورتر مقالات کیوپدیا — انتشار خودکار مقالات از فایل JSON با اسلاگ، چکیده، دسته و سردسته.
 * Version:     8.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-8
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI8_CPT', 'quantum_article' );
define( 'QPI8_TAX', 'quantum_category' );
define( 'QPI8_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi8_load_data() {
	$file = QPI8_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi8_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi8_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi8_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI8_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI8_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi8_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI8_TAX );
 ...
 */
function qpi8_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi8_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi8_authors', $cache, 'qpedia', 300 );
	}
...
function qpi8_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi8_resolve_author( $a['meta']['author'] );
	...
	if ( ! empty( $a['category'] ) ) {
		$ids = qpi8_term_chain( $a['category'], $map );
		if ( $ids ) { wp_set_object_terms( $id, $ids, QPI8_TAX, false ); }
	}
...
function qpi8_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi8_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 8</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi8_run'] ) && check_admin_referer( 'qpi8_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi8_update'] ),
			'draft'  => ! empty( $_POST['qpi8_draft'] ),
		);
		$map = qpi8_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi8_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi8_import' );
	echo '<p><label><input type="checkbox" name="qpi8_update" value="1" checked> به‌روزرسانی مقالاتی که از قبل با همین اسلاگ موجودند</label></p>';
	echo '<p><label><input type="checkbox" name="qpi8_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای ایمپورت', 'primary', 'qpi8_run' );
	echo '</form>';
