<?php
/**
 * Plugin Name: QPedia Importer 13
 ...
 * Version:     13.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-13
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI13_CPT', 'quantum_article' );
define( 'QPI13_TAX', 'quantum_category' );
define( 'QPI13_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi13_load_data() {
	$file = QPI13_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi13_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi13_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi13_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI13_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI13_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi13_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI13_TAX );
 ...
 */
function qpi13_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi13_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi13_authors', $cache, 'qpedia', 300 );
	}
...
function qpi13_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi13_resolve_author( $a['meta']['author'] );
	 ...
	 */
	if ( ! empty( $a['category'] ) ) {
		$current = wp_get_object_terms( $id, QPI13_TAX, array( 'fields' => 'ids' ) );
		if ( is_wp_error( $current ) || empty( $current ) ) {
			$ids = qpi13_term_chain( $a['category'], $map );
			if ( $ids ) { wp_set_object_terms( $id, $ids, QPI13_TAX, false ); }
		}
	}
...
function qpi13_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi13_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 13</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi13_run'] ) && check_admin_referer( 'qpi13_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi13_update'] ),
			'draft'  => ! empty( $_POST['qpi13_draft'] ),
		);
		$map = qpi13_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi13_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi13_import' );
	echo '<p><label><input type="checkbox" name="qpi13_update" value="1" checked> به‌روزرسانی مقالات موجود (این بسته فقط همین کار را می‌کند)</label></p>';
	echo '<p><label><input type="checkbox" name="qpi13_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای بازبینی', 'primary', 'qpi13_run' );
	echo '</form>';
