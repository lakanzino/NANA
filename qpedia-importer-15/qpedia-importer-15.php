<?php
/**
 * Plugin Name: QPedia Importer 15
 ...
 * Version:     15.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-15
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI15_CPT', 'quantum_article' );
define( 'QPI15_TAX', 'quantum_category' );
define( 'QPI15_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi15_load_data() {
	$file = QPI15_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi15_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi15_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi15_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI15_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI15_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi15_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI15_TAX );
 ...
 */
function qpi15_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi15_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi15_authors', $cache, 'qpedia', 300 );
	}
...
function qpi15_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi15_resolve_author( $a['meta']['author'] );
	 ...
	 */
	if ( ! empty( $a['category'] ) ) {
		$current = wp_get_object_terms( $id, QPI15_TAX, array( 'fields' => 'ids' ) );
		if ( is_wp_error( $current ) || empty( $current ) ) {
			$ids = qpi15_term_chain( $a['category'], $map );
			if ( $ids ) { wp_set_object_terms( $id, $ids, QPI15_TAX, false ); }
		}
	}
...
function qpi15_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi15_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 15</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi15_run'] ) && check_admin_referer( 'qpi15_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi15_update'] ),
			'draft'  => ! empty( $_POST['qpi15_draft'] ),
		);
		$map = qpi15_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi15_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi15_import' );
	echo '<p><label><input type="checkbox" name="qpi15_update" value="1" checked> به‌روزرسانی مقالات موجود (این بسته فقط همین کار را می‌کند)</label></p>';
	echo '<p><label><input type="checkbox" name="qpi15_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای بازبینی', 'primary', 'qpi15_run' );
	echo '</form>';
