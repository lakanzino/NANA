<?php
/**
 * Plugin Name: QPedia Thumbnails 28
 * Description: ۴۹ تصویر شاخص نسخهٔ دو (با زیرنویس فارسی) را جایگزین تصویر فعلی مقالات می‌کند و متن جایگزین فارسی استاندارد گوگل می‌نویسد. محتوای هیچ مقاله‌ای تغییر نمی‌کند.
 * Version:     28.0.0
 * Author:      QPedia
 * Text Domain: qpedia-thumbs-28
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPT28_CPT', 'quantum_article' );
define( 'QPT28_DIR', plugin_dir_path( __FILE__ ) );

/* ---------------------------------------------------------------- منو */

add_action( 'admin_menu', function () {
	add_management_page(
		'QPedia Thumbnails 28',
		'QPedia Thumbnails 28',
		'manage_options',
		'qpedia-thumbs-28',
		'qpt28_render_page'
...
function qpt28_load() {
	$file = QPT28_DIR . 'data/manifest.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpt28_nofile', 'فایل data/manifest.json پیدا نشد.' );
	}
	$data = json_decode( file_get_contents( $file ), true );
	if ( ! is_array( $data ) || empty( $data['items'] ) ) {
		return new WP_Error( 'qpt28_badjson', 'ساختار JSON نامعتبر است.' );
	}
	return $data;
}
...
function qpt28_sideload( $path, $filename, $alt, $post_id, $title ) {
	...
	if ( ! empty( $upload['error'] ) ) {
		return new WP_Error( 'qpt28_upload', $upload['error'] );
	}
	...
	if ( is_wp_error( $attach_id ) || ! $attach_id ) {
		return new WP_Error( 'qpt28_attach', 'ثبت پیوست ناموفق بود.' );
	}
...
function qpt28_run( $dry_run = false ) {
	$data = qpt28_load();
	...
	foreach ( $data['items'] as $item ) {
		...
		$attach_id = qpt28_sideload( $path, $file, $alt, $post_id, $ttl );
...
function qpt28_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { return; }

	echo '<div class="wrap" dir="rtl">';
	echo '<h1>QPedia Thumbnails 28</h1>';
	...
	echo '<p>رکورد در فایل: <strong>' . count( $data['items'] )
		...
		. count( glob( QPT28_DIR . 'images/*.webp' ) ) . '</strong></p>';

	$action = '';
	if ( isset( $_POST['qpt28_nonce'] )
		&& wp_verify_nonce( $_POST['qpt28_nonce'], 'qpt28_run' ) ) {
		$action = isset( $_POST['qpt28_dry'] ) ? 'dry' : 'run';
	}

	if ( $action ) {
		@set_time_limit( 300 );
		$log = qpt28_run( 'dry' === $action );
		if ( is_wp_error( $log ) ) {
			echo '<div class="notice notice-error"><p>'
	...
	wp_nonce_field( 'qpt28_run', 'qpt28_nonce' );
	echo '<button type="submit" name="qpt28_dry" value="1" class="button">'
