<?php
/**
 * Plugin Name: QPedia Importer 4
 * Description: ایمپورتر مقالات کیوپدیا — انتشار خودکار مقالات از فایل JSON با اسلاگ، چکیده، دسته و سردسته.
 * Version:     4.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-4
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI4_CPT', 'quantum_article' );
define( 'QPI4_TAX', 'quantum_category' );
define( 'QPI4_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi4_load_data() {
	$file = QPI4_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi4_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi4_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi4_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI4_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI4_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi4_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI4_TAX );
 ...
 */
function qpi4_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi4_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi4_authors', $cache, 'qpedia', 300 );
	}
...
function qpi4_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi4_resolve_author( $a['meta']['author'] );
	...
	if ( ! empty( $a['category'] ) ) {
		$ids = qpi4_term_chain( $a['category'], $map );
		if ( $ids ) { wp_set_object_terms( $id, $ids, QPI4_TAX, false ); }
	}
...
function qpi4_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi4_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 4</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi4_run'] ) && check_admin_referer( 'qpi4_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi4_update'] ),
			'draft'  => ! empty( $_POST['qpi4_draft'] ),
		);
		$map = qpi4_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi4_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi4_import' );
	echo '<p><label><input type="checkbox" name="qpi4_update" value="1" checked> به‌روزرسانی مقالاتی که از قبل با همین اسلاگ موجودند</label></p>';
	echo '<p><label><input type="checkbox" name="qpi4_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای ایمپورت', 'primary', 'qpi4_run' );
	echo '</form>';
