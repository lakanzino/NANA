<?php
/**
 * Plugin Name: QPedia Thumbnails 26
 * Description: ۵۱ تصویر شاخص نسخهٔ دو (با زیرنویس فارسی) را جایگزین تصویر فعلی مقالات می‌کند و متن جایگزین فارسی استاندارد گوگل می‌نویسد. محتوای هیچ مقاله‌ای تغییر نمی‌کند.
 * Version:     26.0.0
 * Author:      QPedia
 * Text Domain: qpedia-thumbs-26
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPT26_CPT', 'quantum_article' );
define( 'QPT26_DIR', plugin_dir_path( __FILE__ ) );

/* ---------------------------------------------------------------- منو */

add_action( 'admin_menu', function () {
	add_management_page(
		'QPedia Thumbnails 26',
		'QPedia Thumbnails 26',
		'manage_options',
		'qpedia-thumbs-26',
		'qpt26_render_page'
...
function qpt26_load() {
	$file = QPT26_DIR . 'data/manifest.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpt26_nofile', 'فایل data/manifest.json پیدا نشد.' );
	}
	$data = json_decode( file_get_contents( $file ), true );
	if ( ! is_array( $data ) || empty( $data['items'] ) ) {
		return new WP_Error( 'qpt26_badjson', 'ساختار JSON نامعتبر است.' );
	}
	return $data;
}
...
function qpt26_sideload( $path, $filename, $alt, $post_id, $title ) {
	...
	if ( ! empty( $upload['error'] ) ) {
		return new WP_Error( 'qpt26_upload', $upload['error'] );
	}
	...
	if ( is_wp_error( $attach_id ) || ! $attach_id ) {
		return new WP_Error( 'qpt26_attach', 'ثبت پیوست ناموفق بود.' );
	}
...
function qpt26_run( $dry_run = false ) {
	$data = qpt26_load();
	...
	foreach ( $data['items'] as $item ) {
		...
		$attach_id = qpt26_sideload( $path, $file, $alt, $post_id, $ttl );
...
function qpt26_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { return; }

	echo '<div class="wrap" dir="rtl">';
	echo '<h1>QPedia Thumbnails 26</h1>';
	...
	echo '<p>رکورد در فایل: <strong>' . count( $data['items'] )
		...
		. count( glob( QPT26_DIR . 'images/*.webp' ) ) . '</strong></p>';

	$action = '';
	if ( isset( $_POST['qpt26_nonce'] )
		&& wp_verify_nonce( $_POST['qpt26_nonce'], 'qpt26_run' ) ) {
		$action = isset( $_POST['qpt26_dry'] ) ? 'dry' : 'run';
	}

	if ( $action ) {
		@set_time_limit( 300 );
		$log = qpt26_run( 'dry' === $action );
		if ( is_wp_error( $log ) ) {
			echo '<div class="notice notice-error"><p>'
	...
	wp_nonce_field( 'qpt26_run', 'qpt26_nonce' );
	echo '<button type="submit" name="qpt26_dry" value="1" class="button">'
