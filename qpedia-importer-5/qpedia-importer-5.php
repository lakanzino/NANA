<?php
/**
 * Plugin Name: QPedia Importer 5
 * Description: ایمپورتر مقالات کیوپدیا — انتشار خودکار مقالات از فایل JSON با اسلاگ، چکیده، دسته و سردسته.
 * Version:     5.0.0
 * Author:      QPedia
 * Text Domain: qpedia-importer-5
 */

if ( ! defined( 'ABSPATH' ) ) { exit; }

define( 'QPI5_CPT', 'quantum_article' );
define( 'QPI5_TAX', 'quantum_category' );
define( 'QPI5_DIR', plugin_dir_path( __FILE__ ) );
...
function qpi5_load_data() {
	$file = QPI5_DIR . 'data/articles.json';
	if ( ! file_exists( $file ) ) {
		return new WP_Error( 'qpi5_nofile', 'فایل data/articles.json پیدا نشد.' );
	}
	$raw = file_get_contents( $file );
	$data = json_decode( $raw, true );
	if ( ! is_array( $data ) || empty( $data['articles'] ) ) {
		return new WP_Error( 'qpi5_badjson', 'ساختار JSON نامعتبر است یا آرایهٔ articles خالی است.' );
	}
	return $data;
}
...
function qpi5_ensure_terms( $categories ) {
	...
	foreach ( array( '', 'child' ) as $pass ) {
		foreach ( $categories as $cat ) {
			...
			$term = get_term_by( 'slug', $slug, QPI5_TAX );
			...
			$new = wp_insert_term( $cat['name'], QPI5_TAX, $args );
			if ( ! is_wp_error( $new ) ) {
 ...
 */
function qpi5_term_chain( $leaf_slug, $map ) {
	$ids  = array();
	$term = get_term_by( 'slug', sanitize_title( $leaf_slug ), QPI5_TAX );
 ...
 */
function qpi5_resolve_author( $name ) {
	$name = trim( wp_strip_all_tags( $name ) );
	if ( '' === $name ) { return 0; }

	$cache = wp_cache_get( 'qpi5_authors', 'qpedia' );
	if ( ! is_array( $cache ) ) {
		$cache = array();
		foreach ( get_users( array( 'fields' => array( 'ID', 'display_name' ) ) ) as $u ) {
			$cache[ $u->display_name ] = (int) $u->ID;
		}
		wp_cache_set( 'qpi5_authors', $cache, 'qpedia', 300 );
	}
...
function qpi5_import_article( $a, $map, $opts ) {
	...
	if ( empty( $postarr['post_author'] ) && ! empty( $a['meta']['author'] ) ) {
		$uid = qpi5_resolve_author( $a['meta']['author'] );
	...
	if ( ! empty( $a['category'] ) ) {
		$ids = qpi5_term_chain( $a['category'], $map );
		if ( $ids ) { wp_set_object_terms( $id, $ids, QPI5_TAX, false ); }
	}
...
function qpi5_render_page() {
	if ( ! current_user_can( 'manage_options' ) ) { wp_die( 'دسترسی مجاز نیست.' ); }

	$data = qpi5_load_data();
	echo '<div class="wrap" dir="rtl"><h1>QPedia Importer 5</h1>';
	...
	$results = array();
	if ( isset( $_POST['qpi5_run'] ) && check_admin_referer( 'qpi5_import' ) ) {
		$opts = array(
			'update' => ! empty( $_POST['qpi5_update'] ),
			'draft'  => ! empty( $_POST['qpi5_draft'] ),
		);
		$map = qpi5_ensure_terms( isset( $data['categories'] ) ? $data['categories'] : array() );
		foreach ( $data['articles'] as $a ) {
			$results[] = qpi5_import_article( $a, $map, $opts );
		}
	}
	...
	wp_nonce_field( 'qpi5_import' );
	echo '<p><label><input type="checkbox" name="qpi5_update" value="1" checked> به‌روزرسانی مقالاتی که از قبل با همین اسلاگ موجودند</label></p>';
	echo '<p><label><input type="checkbox" name="qpi5_draft" value="1"> ایمپورت به‌صورت پیش‌نویس (به‌جای انتشار)</label></p>';
	submit_button( '▶ اجرای ایمپورت', 'primary', 'qpi5_run' );
	echo '</form>';
