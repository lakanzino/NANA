<?php
/**
 * بخش‌های خانه — از page.php وقتی is_front_page() صدا زده می‌شود.
 *
...
if ( have_posts() ) {
	while ( have_posts() ) {
		the_post();
		$qp_home_title = get_the_title();
		$qp_home_html  = apply_filters( 'the_content', get_the_content() );
	}
	rewind_posts();
}
$qp_home_title_raw  = trim( wp_strip_all_tags( (string) $qp_home_title ) );
$qp_home_title_norm = function_exists( 'mb_strtolower' )
	? mb_strtolower( $qp_home_title_raw, 'UTF-8' )
	: strtolower( $qp_home_title_raw );
$qp_generic_titles  = array( 'خانه', 'home', 'homepage', 'صفحه نخست', 'صفحهٔ نخست' );
$qp_use_page_title  = ( '' !== $qp_home_title_norm && ! in_array( $qp_home_title_norm, $qp_generic_titles, true ) );
$qp_hero_h1         = $qp_use_page_title
	? $qp_home_title
	: ( ! empty( $F['hero_title'] ) ? $F['hero_title'] : 'شگفتی‌های دنیای کوانتوم را ساده، دقیق و بی‌اغراق کشف کنید' );
$qp_has_page_body = ( '' !== trim( wp_strip_all_tags( (string) $qp_home_html ) ) );
...
<main id="primary" class="site-main">
	<div class="container qp-front">
		...
		<section class="qp-front-hero">
			...
			<?php endif; ?>
			<h1 class="qp-front-hero__title"><?php echo esc_html( $qp_hero_h1 ); ?></h1>
			<?php if ( $qp_has_page_body ) : ?>
				<div class="qp-front-hero__desc qp-front-hero__content entry-content"><?php echo $qp_home_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
			<?php elseif ( ! empty( $F['hero_desc'] ) ) : ?>
